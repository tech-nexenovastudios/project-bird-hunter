...
/////
///// FLOW:
/////   Select cannon → costText updates live showing current upgrade cost.
/////   Press Upgrade → deducts Gold immediately, upgrades, saves to cloud.
/////   Can't afford  → fading "Not enough Gold!" text appears on panel.
/////
///// FILL BARS (9-Sliced):
/////   Each stat has two overlapping fill images (Image Type = Sliced):
/////     - CurrentFill (bright green): shows current stat level, animates on upgrade.
/////     - NextFill (light green): sits behind CurrentFill, shows where the
/////       stat will reach after the next upgrade (preview).
/////   Both preserve rounded caps at any width thanks to 9-slice rendering.
/////
///// SPRITE SETUP:
/////   1. Open each fill sprite in Sprite Editor.
/////   2. Set Border L/R to fully contain the rounded ends (e.g. L=12, R=12).
/////   3. In Inspector, set Image Type → Sliced on every fill Image.
/////   4. Hierarchy: NextFill first (behind), CurrentFill second (in front).
/////   5. Both anchored to stretch full parent: anchorMin(0,0) anchorMax(1,1).
///// </summary>
...
//    [Header("Minimum Fill Ratio (prevents rounded caps from overlapping)")]
//    [SerializeField] private float minFillRatio = 0.05f;
...
//    // Cached ratios for smooth animation (animate FROM these → new values)
//    private float prevDamageRatio;
//    private float prevHealthRatio;
//    private float prevFireRateRatio;
...
//    // ═════════════════════════════════════════════════════════════════════════
//    // Stats Display (text labels + snap fills)
//    // ═════════════════════════════════════════════════════════════════════════
...
//        // Snap fill bars to current values (no animation)
//        SnapFills();
//    }

//    // ═════════════════════════════════════════════════════════════════════════
//    // 9-Slice Fill Bar System
//    // ═════════════════════════════════════════════════════════════════════════

//    /// <summary>
//    /// Sets a Sliced image's visible width by adjusting anchorMax.x.
//    /// Because Image Type = Sliced, Unity preserves the rounded caps
//    /// defined in the sprite's Border settings and only stretches the middle.
//    /// </summary>
//    private void SetFill(Image img, float ratio)
//    {
//        if (img == null) return;

//        // Hide entirely if ratio is effectively zero
//        if (ratio <= 0.001f)
//        {
//            img.enabled = false;
//            return;
...
//        // Clamp to minimum so left + right rounded caps never overlap
//        float clamped = Mathf.Clamp(ratio, minFillRatio, 1f);
...
//    /// <summary>
//    /// Immediately snaps all fill bars to match current cannon stats.
//    /// Called on cannon selection and initial load (no animation needed).
//    /// </summary>
...
//        int level = data.cannonLevel;

//        // Maximum possible values at max level (used to normalize 0→1)
//        float maxD = stats._baseBulletDamage + stats.damageIncrement * maxCannonLevel;
//        float maxH = stats._baseMaxHealth + stats.healthIncrement * maxCannonLevel;
//        float maxF = stats._baseFireRate + stats.fireRateIncrement * maxCannonLevel;

//        // Current ratios
//        float curD = stats.bulletDamage / maxD;
//        float curH = stats.maxHealth / maxH;
//        float curF = stats.fireRate / maxF;

//        // Snap current fills (bright green)
//        SetFill(damageFillCurrent, curD);
//        SetFill(healthFillCurrent, curH);
//        SetFill(fireRateFillCurrent, curF);

//        // Next-level preview fills (light green, one increment ahead)
//        if (level < maxCannonLevel)
//        {
//            float nextD = (stats.bulletDamage + stats.damageIncrement) / maxD;
//            float nextH = (stats.maxHealth + stats.healthIncrement) / maxH;
//            float nextF = (stats.fireRate + stats.fireRateIncrement) / maxF;

//            SetFill(damageFillNext, nextD);
//            SetFill(healthFillNext, nextH);
//            SetFill(fireRateFillNext, nextF);
//        }
//        else
//        {
//            // At max level — next preview matches current (no preview visible)
//            SetFill(damageFillNext, curD);
//            SetFill(healthFillNext, curH);
//            SetFill(fireRateFillNext, curF);
//        }

//        // Cache for animation origin on next upgrade
//        prevDamageRatio = curD;
//        prevHealthRatio = curH;
//        prevFireRateRatio = curF;
//    }

//    /// <summary>
//    /// Smoothly animates fill bars from previous ratios to new ratios
//    /// after a successful upgrade. Uses SmoothStep for a polished feel.
//    /// Called instead of SnapFills when upgrading the currently selected cannon.
//    /// </summary>
...
//        float newD = stats.bulletDamage / maxD;
//        float newH = stats.maxHealth / maxH;
//        float newF = stats.fireRate / maxF;

//        // ── Step 1: Animate current fills (bright green) from old → new ───
//        float elapsed = 0f;
...
//            SetFill(damageFillCurrent, Mathf.Lerp(prevDamageRatio, newD, t));
//            SetFill(healthFillCurrent, Mathf.Lerp(prevHealthRatio, newH, t));
//            SetFill(fireRateFillCurrent, Mathf.Lerp(prevFireRateRatio, newF, t));

//            await UniTask.Yield();
//        }

//        // Snap to exact final values
//        SetFill(damageFillCurrent, newD);
//        SetFill(healthFillCurrent, newH);
//        SetFill(fireRateFillCurrent, newF);

//        // ── Step 2: Update next-level preview (light green) ───────────────
//        if (level < maxCannonLevel)
//        {
//            float nextD = (stats.bulletDamage + stats.damageIncrement) / maxD;
//            float nextH = (stats.maxHealth + stats.healthIncrement) / maxH;
...
//        else
//        {
//            SetFill(damageFillNext, newD);
//            SetFill(healthFillNext, newH);
//            SetFill(fireRateFillNext, newF);
//        }

//        // Cache for next upgrade animation
//        prevDamageRatio = newD;
//        prevHealthRatio = newH;
//        prevFireRateRatio = newF;
//    }
...
//    /// <summary>
//    /// Updates the cost container whenever a cannon is selected.
//    /// Hides the entire container at max level (icon + text hidden together).
//    /// Shows plain number e.g. "500" — icon is handled by the container itself.
//    /// </summary>
...
//        // Update level badge on the cannon item
//        if (index < cannonItems.Count && cannonItems[index].levelText != null)
//            cannonItems[index].levelText.text = $"{data.cannonLevel}";

//        // Refresh text labels, cost, and animate fill bars if still selected
//        if (index == selectedIndex)
//        {
//            // Update text labels (name, level, description)
//            if (selectedCannonImage != null) selectedCannonImage.sprite = data.cannonSprite;
//            if (statsLevelText != null) statsLevelText.text = $"Level {data.cannonLevel}";
...
//            // Animate fill bars smoothly from old → new values
