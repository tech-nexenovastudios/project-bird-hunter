...
public abstract class BaseCannon<TBullet, TBehaviour> : MonoBehaviour, IDamageable, ICannonBase
        ...
        where TBehaviour : IBulletBehaviour<TBullet>, new()
    {
        ...
        /// <summary>
        /// Override in each cannon — only Fire() changes per cannon type.
        /// </summary>
        protected abstract void Fire(Vector2 position, Vector2 direction);

        // ─────────────────────────────────────────
        // SpawnBullet helper — typed, no casting
        // ─────────────────────────────────────────
        protected TBullet SpawnBullet(Vector2 position, Vector2 direction)
        {
            return BulletManager.Instance.SpawnBullet<TBullet>(
                CannonStats.baseBulletConfig,   // ← was CannonStats.baseBulletType
                position,
                direction
            );
        }

        // Overload for spawning a DIFFERENT bullet type from within a cannon
        protected TOther SpawnBullet<TOther>(Vector2 position, Vector2 direction)
            where TOther : MonoBehaviour, IBullet
        {
            return BulletManager.Instance.SpawnBullet<TOther>(
                CannonStats.baseBulletConfig,
                position,
                direction
            );
        }
        ...
        protected virtual void HandleMovement()
        {
            ...
            if (wheels != null && wheels.Length > 0)
            {
                ...
                float rot  = -(dist / (2 * Mathf.PI * wheelRadius)) * 360f;
        ...
        // ─────────────────────────────────────────
        // Collision relay (ICannonBase implementation)
        // ─────────────────────────────────────────
        public void ReceivePartCollisionEnter(CannonPartCollider part, Collision2D col)
        {
            ...
            HandleCollisionEnter(BuildData(col.gameObject, col.contacts[0].point,
                col.contacts[0].normal, col.relativeVelocity.magnitude,
                CollisionEventType.CollisionEnter, part));
        }
        ...
        private CannonCollisionData BuildData(GameObject other, Vector2 point,
            Vector2 normal, float force, CollisionEventType type, CannonPartCollider hitPart)
            => new()
            {
                sourceCannon  = this,
                hitPart       = hitPart,
