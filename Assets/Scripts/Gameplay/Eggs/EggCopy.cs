...
namespace Gameplay.Eggs
{
    ...
    public class EggCopy : MonoBehaviour
    {
        ...
        public Action<EggCopy> OnDestroyed;
        public Action<EggCopy> OnTrySplit;
        ...
        // Self-clear drift state. _isFrozen kept for API compat — semantic is "in self-clear drift".
        ...
        {
            if (mainCam == null) { _driftMinY = _bottomBound; return; }
            float worldBottom = mainCam.ViewportToWorldPoint(Vector3.zero).y;
            float worldTop    = mainCam.ViewportToWorldPoint(Vector3.one).y;
            _driftMinY = worldBottom + (worldTop - worldBottom) * driftMinHeightPercent;
        }
        ...
        private void PlayMinorHitVFX()
        {
            ...
            if (_minorHitVFXInstance == null)
            {
                ...
                // Force one-shot so emission stops on its own even if the prefab is authored as looping.
