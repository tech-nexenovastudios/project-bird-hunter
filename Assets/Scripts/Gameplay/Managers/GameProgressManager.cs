...
namespace Gameplay.Managers
{
    public class GameProgressManager : MonoBehaviour
    {
        ...
        [Header("Powerup Pool")] 
        public PowerupConfig[] allPowerups; // All 30 powerup ScriptableObjects
        ...
        public void CompleteLevel(int scoreAchieved)
        {
            ...
            // Level in chapter is 0-indexed for spin check
        ...
        {
            var options = _progress.GetAvailablePowerUpForSpin(  // ← was GetAvailablePowerUpForSpin
        ...
        // ─────────────────────────────────────────────
        // Player Picks a Powerup from Spin
        // ─────────────────────────────────────────────
        // FIXED: Add slotIndex parameter**
        public void PlayerSelectedPowerup(PowerupConfig powerup)
        {
            ...
            SaveProgress();
            OnProgressChanged?.Invoke(_progress);
            Debug.Log($"✅ Equipped [{powerup.rarity}] {powerup.displayName} → Category {powerup.category}");
        }

        // ─────────────────────────────────────────────
        // FIXED: ApplyPowerUpsToCurrentCannon
        // ─────────────────────────────────────────────
        public void ApplyPowerUpsToCurrentCannon(GameObject cannon)
        {
            // Apply all non-empty slots
            foreach (var slot in _progress.chapterSlots)
            {
                if (!string.IsNullOrEmpty(slot.equippedPowerupId) && !slot.isOnCooldown)
                {
                    var powerup = Array.Find(allPowerups, p => p.id == slot.equippedPowerupId);
                    if (powerup != null)
                    {
                        //powerup.ApplyEffect(cannon); // Your effect implementation
                    }
                }
            }
        }
